...
2. On the toolbar, choose **Create card**—content fills Weave’s memory card creation flow.
3. After save, cards can carry reader traces to jump back from study UI.
4. Without Weave installed, the entry is unavailable or prompts missing capability.
...
2. Custom AI splitting depends on split features configured on the Weave side.
...
> Note: Reader AI is not blocked by **reader premium** alone; without Weave and API keys, the panel cannot help.

## 4. Vocabulary List
Adding selections to the vocabulary list and marking words in body text: see `Er-03b 生词标注与词汇表`. Review still happens in Weave memory study.

## 5. How Licensing Works
1. Reader premium: Timeline, precise bidirectional tracing, paragraph mode, reading lists, etc.—reader activation code, or inherit from activated Weave.
...
3. Reader only, no Weave: Integration entries lack a host.


## 6. Recommended Combinations
1. **Read and take notes only**: Reader + Markdown / Canvas—Weave optional.
2. **Read and spaced review**: Reader + Weave—highlight to card or collect vocabulary.
